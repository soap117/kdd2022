...
save_data = torch.load('./results/' + config.data_file.replace('.pkl', '_models_edit_dual_pure.pkl').replace('data/', ''), map_location=config.device)
...
def pipieline(path_from):
    ...
    for src, tar in zip(srcs, tars):
        ...
        logits_edit, hidden_edits = modeld(input_ids=decoder_ids, decoder_input_ids=target_ids,
                                                          ...
                                                          input_edits=edit_sens_token_ids, org_ids=None,
                                                          force_ratio=0.0, eval=True, clean_indication=clean_indication)

        _, predictions = torch.max(logits_edit, dim=-1)

        decoder_inputs = config.tokenizer_editplus([src], return_tensors="pt", padding=True)
        decoder_ids = decoder_inputs['input_ids']
    ...
    with open('../data/test/my_results_edit_para_dual_pure.pkl', 'wb') as f:
