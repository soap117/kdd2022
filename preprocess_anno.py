...
if os.path.exists(config.data_file_anno):
    with open(config.data_file_anno, 'rb') as f:
...
for name in names:
    ...
    for path in plist:
        if path in path2file:
            temp += path2file[path]
    with open(config.data_file_anno.replace('.pkl', '_%s_dataset_raw.pkl' %(name)), 'wb') as f:
        pickle.dump(temp, f)
