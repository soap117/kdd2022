    ...
    def forward(self, input_edits, hidden_org,encoder_outputs_org, org_ids, simp_sent,teacher_forcing_ratio=1.):
        ...
        if use_teacher_forcing:
            ...
            for t in range(nsteps-1):
                ...
                inds = torch.LongTensor(np.clip(np.array(counter_for_keep_del)-1, 0, 512))
                dummy = inds.view(-1, 1, 1)
                dummy = dummy.expand(dummy.size(0), dummy.size(1), encoder_outputs_org.size(2)).cuda()
                c2 = encoder_outputs_org.gather(1, dummy)

                inds = torch.LongTensor(np.clip(np.array(counter_for_keep_del)-2, 0, 512))
                dummy = inds.view(-1, 1, 1)
                dummy = dummy.expand(dummy.size(0), dummy.size(1), encoder_outputs_org.size(2)).cuda()
...
class EditDecoderRNNWA(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, n_layers=1, embedding=None):
        super(EditDecoderRNNWA, self).__init__()
        self.hidden_size = hidden_size
        self.embedding_dim = embedding_dim
        self.vocab_size = vocab_size
        self.n_layers = n_layers
    ...
    def forward(self, input_edits, hidden_org,encoder_outputs_org, org_ids, simp_sent,teacher_forcing_ratio=1.):
        ...
        if use_teacher_forcing:
            ...
            for t in range(nsteps-1):
                ...
                c_word = output_words.gather(1, dummy)
                dummy = inds.view(-1, 1, 1)
                dummy = dummy.expand(dummy.size(0), dummy.size(1), attn_applied_org.size(2)).cuda()
                attn_applied_org_t = attn_applied_org.gather(1, dummy)
...
class EditDecoderRNNDual(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, n_layers=1, embedding=None):
        super(EditDecoderRNNDual, self).__init__()
        self.hidden_size = hidden_size
        self.embedding_dim = embedding_dim
        self.vocab_size = vocab_size
        self.n_layers = n_layers
        ...
        self.attn_MLP_action = nn.Sequential(nn.Linear(hidden_size * 4, embedding_dim),
                                          nn.Tanh())
        self.attn_MLP_edit = nn.Sequential(nn.Linear(hidden_size * 4, embedding_dim),
                                      nn.Tanh())
        self.out_edit = nn.Linear(embedding_dim, self.vocab_size)
        self.out_edit.weight.data = self.embedding.weight.data[:self.vocab_size]
    ...
    def forward(self, input_edits, hidden_org,encoder_outputs_org, org_ids, simp_sent,teacher_forcing_ratio=1.):
        ...
        if use_teacher_forcing:
            ...
            attn_weights_org_edits = F.softmax(logits_org, dim=-1)  # bsz x nsteps x encsteps
            attn_applied_org_edits = torch.bmm(attn_weights_org_edits, encoder_outputs_org)  # bsz x nsteps x nhid

            key_org_actions = self.attn_Projection_org(output_actions)  # bsz x nsteps x nhid MIGHT USE WORD HERE
            logits_org = torch.bmm(key_org_actions, encoder_outputs_org.transpose(1, 2))  # bsz x nsteps x encsteps
            ...
            for t in range(nsteps-1):
                ...
                decoder_output_action_t = output_actions[:, t:t + 1, :]
                attn_applied_org_action_t = attn_applied_org_actions[:, t:t + 1, :]
            ...
            while t < tt:
                ...
                logits_org = torch.bmm(key_org_edits, encoder_outputs_org.transpose(1, 2))  # bsz x nsteps x encsteps
                attn_weights_org_edit_t = F.softmax(logits_org, dim=-1)  # bsz x nsteps x encsteps
                attn_applied_org_edit_t = torch.bmm(attn_weights_org_edit_t, encoder_outputs_org)  # bsz x nsteps x nhid

                key_org_actions = self.attn_Projection_org(output_actions)  # bsz x nsteps x nhid
                logits_org = torch.bmm(key_org_actions, encoder_outputs_org.transpose(1, 2))  # bsz x nsteps x encsteps
                attn_weights_org_action_t = F.softmax(logits_org, dim=-1)  # bsz x nsteps x encsteps
                attn_applied_org_action_t = torch.bmm(attn_weights_org_action_t, encoder_outputs_org)
                ...
                dummy_2 = inds.view(-1, 1).cuda()
                org_t = org_ids.gather(1, dummy_2)
                hidden_words = self.execute_batch(pred_edit, org_t, hidden_words)  # we give the editted subsequence
                # hidden_words = self.execute_batch(pred_action, org_t, hidden_org)  #here we only give the word
                ...
                if check:
                    break
        decoder_action_out = torch.cat(decoder_action_out, dim=1)
        decoder_edit_out = torch.cat(decoder_edit_out, dim=1)
        return (decoder_action_out, decoder_edit_out), hidden_edits
...
class EditDecoderRNNDualSI(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, n_layers=1, embedding=None):
        super(EditDecoderRNNDualSI, self).__init__()
        self.hidden_size = hidden_size
        self.embedding_dim = embedding_dim
        self.vocab_size = vocab_size
        self.n_layers = n_layers
...
class EditDecoderRNNDualSICRoss(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, n_layers=1, embedding=None):
        super(EditDecoderRNNDualSICRoss, self).__init__()
        self.hidden_size = hidden_size
        self.embedding_dim = embedding_dim
        self.vocab_size = vocab_size
        self.n_layers = n_layers
        ...
        self.attn_MLP_action = nn.Sequential(nn.Linear(hidden_size * 5, embedding_dim),
                                          nn.Tanh())
        self.attn_MLP_edit = nn.Sequential(nn.Linear(hidden_size * 5, embedding_dim),
                                      nn.Tanh())
        self.out_edit = nn.Linear(embedding_dim, self.vocab_size)
        self.out_edit.weight.data = self.embedding.weight.data[:self.vocab_size]
...
class EditDecoderRNNDualCRoss(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, n_layers=1, embedding=None):
        super(EditDecoderRNNDualCRoss, self).__init__()
        self.hidden_size = hidden_size
        self.embedding_dim = embedding_dim
        self.vocab_size = vocab_size
        self.n_layers = n_layers
    ...
    def forward(self, input_edits, hidden_org, encoder_outputs_org, org_ids, simp_sent, teacher_forcing_ratio=1.):
        ...
        input_actions = torch.where(
            (input_edits != 0) & (input_edits != 1) & (input_edits != 2) & (input_edits != 3) & (input_edits != 4) & (
                        input_edits != 5), input_actions, input_edits)
        if use_teacher_forcing:
            embedded_edits = self.embedding(input_edits)
            output_edits, hidden_edits = self.rnn_edits(embedded_edits, hidden_org)
...
class EditNTSDualSI(nn.Module):
    def __init__(self, config, n_layers=2):
        super(EditNTSDualSI, self).__init__()
...
class EditNTSWAT(nn.Module):
    def __init__(self, config, n_layers=2):
        super(EditNTSWAT, self).__init__()
        self.embedding = nn.Embedding(config.vocab_size, config.embedding_dim)
        if not(config.pretrained_embedding is None):
            print('load pre-trained embeddings')
            self.embedding.weight.data.copy_(torch.from_numpy(config.pretrained_embedding))
...
class EditNTSDualSICross(nn.Module):
    def __init__(self, config, n_layers=2):
        super(EditNTSDualSICross, self).__init__()
        self.embedding = nn.Embedding(config.vocab_size, config.embedding_dim)
        if not(config.pretrained_embedding is None):
            print('load pre-trained embeddings')
            self.embedding.weight.data.copy_(torch.from_numpy(config.pretrained_embedding))
        ...
        self.decoder = EditDecoderRNNDualSICRoss(config.vocab_size, config.embedding_dim, config.word_hidden_units * 2,
                                      n_layers, self.embedding)
...
class EditNTSDualCross(nn.Module):
    def __init__(self, config, n_layers=2):
        super(EditNTSDualCross, self).__init__()
        self.embedding = nn.Embedding(config.vocab_size, config.embedding_dim)
        if not(config.pretrained_embedding is None):
            print('load pre-trained embeddings')
            self.embedding.weight.data.copy_(torch.from_numpy(config.pretrained_embedding))
        ...
        self.decoder = EditDecoderRNNDualCRoss(config.vocab_size, config.embedding_dim, config.word_hidden_units * 2,
                                      n_layers, self.embedding)
...
class EditNTSFar(nn.Module):
    def __init__(self, config, n_layers=2):
        super(EditNTSFar, self).__init__()
        self.embedding = nn.Embedding(config.vocab_size, config.embedding_dim)
        if not(config.pretrained_embedding is None):
            print('load pre-trained embeddings')
            self.embedding.weight.data.copy_(torch.from_numpy(config.pretrained_embedding))
