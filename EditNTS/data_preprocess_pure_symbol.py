...
def find_location(src_token, key):
    key_token = [x.word for x in posseg.cut(key)]
    for i in range(len(src_token)-len(key_token)+1):
        flag = True
        for k in range(len(key_token)):
            if key_token[k] != src_token[i+k] and key_token[k] not in src_token[i+k]:
                flag = False
                break
        if flag:
            return (i, i+len(key_token))
    else:
        return (-1,-1)

def modify_sentence_direct(srcs, tars, query_groups, is_train):
    new_srcs_indication = []
    new_query_groups =[]
    for src, tar, query_group in zip(srcs, tars, query_groups):
        used = set()
        new_query_group = []
        temp_indication = np.array([8 for x in range(len(src))])
        for query_data in query_group:
            query_data = list(query_data)
            query_data[0] = query_data[0].lower()
            query = query_data[0]
            query = query.replace('(', '（')
            query = query.replace(')', '）')
            query = query.replace('（', '')
            ...
            loc = find_location(src, query)
            loc2 = find_location(tar, query)
            if (loc != (-1,-1) and not is_train) or (loc != (-1,-1) and loc2 != (-1,-1)):
                temp_indication[loc[0]:loc[1]] = 9
                new_query_group.append(query_data)
                used.add(query)
        new_query_groups.append(new_query_group)
        new_srcs_indication.append(temp_indication)
    return new_query_groups, new_srcs_indication
...
def process_raw_data(train_data, is_train):
    ...
    querys_group, comp_txt_indication = modify_sentence_direct(comp_txt, simp_txt, querys_group, is_train)
    df['comp_indication_ids'] = comp_txt_indication
    ...
    def get_vocab(df):
        ...
        f = open('./vocab_data/vocab_pure.txt', 'w', encoding='utf-8')
...
def editnet_data_to_editnetID(df,output_path):
    ...
    for i,example in df.iterrows():
        ...
        ex=[example['comp_tokens'], comp_id,
         ...
         example['comp_pos_tags'],example['comp_pos_ids'],example['comp_indication_ids'],
         example['querys'], example['querys_ori'], example['query_context'],example['infer_titles']
         ]
        out_list.append(ex)
    outdf = pd.DataFrame(out_list, columns=['comp_tokens','comp_ids', 'simp_tokens','simp_ids',
                                            ...
                                            'comp_indication_ids', 'querys', 'querys_ori','query_context','infer_titles'])
...
editnet_data_to_editnetID(df, './mydata/test_pure_in.df.filtered.pos')