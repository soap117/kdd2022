...
from editnts import EditNTSDualSI, EditNTSWAT, EditNTSDualSICross, EditNTS, EditNTSDualCross, EditNTSDual
...
def training(edit_net,nepochs, args, vocab, print_every=100, check_every=500):
    eval_dataset = data.Dataset(args.data_path + 'val_full_wo.df.filtered.pos') # load eval dataset
    test_dataset = data.Dataset(args.data_path + 'test_full_wo.df.filtered.pos')  # load eval dataset
    ...
    for epoch in range(nepochs):
        ...
        if os.path.isfile(args.data_path+'train_full_wo.df.filtered.pos'):
            train_dataset = data.Dataset(args.data_path + 'train_full_wo.df.filtered.pos')
        else:  # iter chunks and vocab_data
            train_dataset = data.Datachunk(args.data_path + 'train_full_wo.df.filtered.pos')
...
def main():
    ...
    print('*'*10)
    vocab = data.Vocab()
    vocab.add_vocab_from_file(args.vocab_path+'vocab_full.txt', args.vocab_size)
    ...
    with open('my_results_EditNT_full_mark_wo.pkl', 'wb') as f:
