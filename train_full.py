...
def build(config):
    ...
    save_data = torch.load('./results/' + config.data_file_anno_old.replace('.pkl', '_models_full.pkl').replace('data/', ''))
    ...
    optimizer_encoder = AdamW(modele.parameters(), 1e-5)
    optimizer_decoder = AdamW(modeld.parameters(), 2e-5)
