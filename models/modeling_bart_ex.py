...
def _make_causal_mask(input_ids_shape: torch.Size, dtype: torch.dtype, past_key_values_length: int = 0, h_indicator: torch.LongTensor = None):
    """
    ...
    if h_indicator is not None:
        mask_cond += h_indicator
...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        attention_adjust: Optional[torch.Tensor] = None,
...
class BartEncoderLayer(nn.Module):
    def __init__(self, config: BartConfig, idx=0):
        super().__init__()
...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([BartEncoderLayer(config, idx=idx) for idx in range(config.encoder_layers)])
...
class BartDecoder(BartPretrainedModel):
    ...
    def _prepare_decoder_attention_mask_with_hidden_annotation(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        # create causal mask
        # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
        combined_attention_mask = None
        if input_shape[-1] > 1:
